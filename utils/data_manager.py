...
class DataManager(object):
    ...
    @property
    def nb_tasks(self):
        return len(self._increments)    # task

    def get_task_size(self, task):
        return self._increments[task]   

    def get_total_classnum(self):
        return len(self._class_order)   # class
    ...
    def get_dataset(self, indices, source, mode, shot=None, appendent=None, ret_data=False, m_rate=None):
        ...
        if mode == "train":     # transforms.Compose transform
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        # Order
        order = [i for i in range(len(np.unique(self._train_targets)))]  # list
        if shuffle:     
            np.random.seed(seed)
